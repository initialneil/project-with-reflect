...
- **checkin `[<name>]`** — **pick up a project/connection and get ready** (the natural next step after
  ...
  `mcp__ccd_session_mgmt__set_session_title` renames *other* sessions, **not the current one** (rejects
  self-rename) — a session **cannot auto-title itself**; the user double-clicks the sidebar title. Don't call
