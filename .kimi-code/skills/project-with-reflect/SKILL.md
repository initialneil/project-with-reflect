...
description: >-
  ...
  /skill:<project> or /skill:<connection>; it auto-logs the moments that matter; reflect distills those
  logs into lean, readable lessons so the next session — yours or any agent's — starts smarter
  instead of repeating mistakes. Use when the user types /skill:project-with-reflect,
  /skill:register-project, /skill:register-machine, /skill:register-device, /skill:register-api,
  /skill:register-mcp, /skill:register-knowledge, /skill:register-agent, /skill:update, or
  /skill:meta-reflect, or asks to register / manage / reflect-on a project, machine, device, API,
  MCP, or knowledge note, or wants per-project persistent memory + a log->reflect->improve loop.
arguments:
  - action
---
...
## Kimi Code adapter note

This is the Kimi Code–facing mirror of the canonical skill. Interpret Claude Code slash-command
names such as `/project-with-reflect`, `/register-project`, and `/<project>` as user intents or
skill handles; in Kimi Code they map to `/skill:<name>` (e.g. `/skill:project-with-reflect status`,
`/skill:register-project myapp ~/code/myapp`, `/skill:<project>`). When the canonical text says
`AskUserQuestion`, use Kimi Code's AskUserQuestion tool.

The scripts resolve via `${KIMI_SKILL_DIR}` (this skill's directory, symlinked to the canonical
`scripts/` and `templates/`). All scripts below are `${KIMI_SKILL_DIR}/scripts/*`.

Auto-log nudge hooks for Kimi Code live in `~/.kimi-code/config.toml` (`[[hooks]]` entries pointing
at `.kimi-code/hooks/project-with-reflect.sh`), installed by `scripts/install-kimi-code.sh`.
...
> `~/.agents/skills/project-with-reflect` for Kimi Code, `~/.claude/skills/project-with-reflect`
> for Claude Code, or `~/.codex/skills/project-with-reflect` for Codex). All scripts below are
> `SK/scripts/*`. Run `SK/scripts/bootstrap.sh` once before anything else.
...
Bare `/skill:project-with-reflect` → `help`.
...
- **checkin `[<name>]`** — **pick up a project/connection and get ready** (the natural next step after
  ...
  via its native thread-title tool, resolving the active thread with a broad recent-thread lookup if the
  current thread id isn't in context. **Claude Desktop/web**: Claude Code's title tool
  `mcp__ccd_session_mgmt__set_session_title` renames *other* sessions, **not the current one** (it rejects
  self-rename) — so a session **cannot auto-title itself**; the user double-clicks the sidebar title instead. Don't call
  it on the current session. It then ends with a `status`
...
- **doctor `[<name>]`** — check/repair the local install: root pointer, root directory shape,
  registry readability, and Claude/Codex/Kimi Code user-scope skill links for already-registered
  projects/connections. Run `SK/scripts/doctor.sh [<name>]`. Use when a project created in Claude
  Code exists in `$ROOT/projects/<name>` but Codex or Kimi Code cannot discover `/<name>` /
  `/skill:<name>`, or after moving/syncing the root across machines.
- **register-project `<name> [path]`** — **the user describes it in plain language; you parse
  ...
  **workstream_mode** (worktree | in-repo | logical), **local or remote** (remote → the ssh connection
  ...
  state). Generates `/<name>` (Kimi: `/skill:<name>`). If "import", run `/<name> bootstrap` (it reads every root, over the host
...
the supported user-scope skill directories: `~/.claude/skills/<name>` for Claude Code,
`~/.codex/skills/<name>` for Codex, `~/.agents/skills/<name>` for Kimi Code) that you operate via
`/<name>` (Kimi Code: `/skill:<name>`) and that accrues learned quirks via
...
- **register-knowledge `<slug>`** — a **global knowledge module as a real skill** (knowledge IS a
  ...
  dirs so **`/<slug>` autocompletes + auto-triggers everywhere** (Kimi: `/skill:<slug>`). The slug **must not collide with
...
a repo (e.g. local Docker) is project code, not a connection — only register a connection for
...
`${CODEX_HOME:-~/.codex}/skills/<command>/SKILL.md` by `install-codex-command-skills.sh`. For Kimi Code,
the same aliases are mirrored into `~/.agents/skills/<command>/SKILL.md` by
`install-kimi-command-skills.sh`. `bootstrap`
...
`/<project>-<workstream> checkin` discoverable as Codex/Kimi skills. Eval quick handles are also installed as
user-scope skills when the short name is not already owned by another project. The templates,
`gen-command.sh`, and the Codex/Kimi command-skill installers must stay in sync when adding an action.
...
your summary — reload the folder-notes plugin (or restart Obsidian) to see it — otherwise don't
mention folder-notes at all. (**Knowledge** folders follow the same convention: `knowledge/<slug>/<slug>.md` is the folder
...
(`SK/scripts/hook-autolog.sh`, declared in the canonical SKILL's frontmatter for Claude Code and
mirrored by `.codex/hooks.json` for Codex and by `[[hooks]]` entries in `~/.kimi-code/config.toml`
for Kimi Code) nudge the harness, not the model's memory:
- **UserPromptSubmit**: when a prompt looks like substantial work inside a registered project, remind the
  agent to start with `/<project> checkin` (Kimi: `/skill:<project> checkin`).
