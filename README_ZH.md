...
learning loop；可读性思路借鉴 `grounding-rules` skill。**没有硬性 cap**——精简来自
...
projects/<name>/
  ...
  status.md            人类看的 dashboard —— 由 reflect 重新生成
...
**单个 project**（`/<name>`）：`status` · `list` · `help` · `reflect [--reground]` ·
...
1. **先加载，再提议** —— 读 `status.md` + `decisions.md` + 匹配的 rule modules。
...
/iot build && /iot flash && /iot monitor   # 把 server endpoint 编进固件、通过 USB flash、看它连上 server
```

## Reflect = bounded update

`reflect` 把新的 log 条目折叠进对应的 `rules/<topic>.md` + `decisions.md`，修正错误
的 rules，**太长就拆 module**，重新生成 `status.md`，归档已消化的 log，并报告改了
什么。`--reground` 会对某个 module 做完整重写。没有 cap；可读性说了算。

## 发布 Releasing（维护者）

本 repo 自带一个仓库内的 `/release` skill（`.claude/skills/release`）。`/release
[patch|minor|major] "note"` 会 bump 版本、更新 CHANGELOG + manifests、打 tag、push，
并创建 GitHub release。bump 映射：**major** = 面向用户的大改动，**minor** = major fix，
**patch** = minor fix（默认）。
