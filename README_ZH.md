...
/myapp checkin                      # 接手 "myapp"：加载它的上下文，询问工作目录
                                    #   （cd 进 lane 文件夹 / 代码目录），然后 recap —— 立刻就绪
/myapp checkin train-v2            # …… 或直接 checkin 进某条具体 lane
```

`status` 是**智能简报**（Where · Recap · TODO · Workstreams · flags），不是 dump。`checkin` 是
**一个 working session 的入口** —— 它加载上下文、处理 cwd、**静默地把终端 tab 标题设为 project + lane**
...
lessons + decisions。之后在任意位置开一个 session，说 **`/myapp work on <lane>`** —— 它会先征求你同意
...
**单个 project**（`/<name>`）：`checkin [<lane>]` · `status` · `bootstrap` · `list` · `help` ·
...
```
/app register-branch v090 --base v080 --track-only   # 仅 lineage：v090 tracks v080（共享分支 / PR 目标，不开 worktree）
/app register-branch v090-bug-fix --base v090        # 在 app/.claude/worktrees/v090-bug-fix 开 worktree，从 origin/v090 fork
...
**任何 rebase 前都会先询问**，让你绝不会 PR 到一个 stale 的目标。worktree lane 默认落在
`<repo>/.claude/worktrees/<branch>`（自动创建、不进 git）；一个 workstream 是**可复用的 lane**，不是一次性的。
