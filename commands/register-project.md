...
dataset repo at …"). Only **AskUserQuestion** for what's genuinely missing or ambiguous — mode
(central|in-repo, default central), workstream_mode (worktree|in-repo), whether to import existing docs.
...
- If the project is **remote** and its host isn't a registered connection yet, **register it first**
  (`register-machine <host>` — point the user at a key-based `Host` block in `~/.ssh/config`); remote
  forces central mode (no local checkout).
- Run `scripts/register-project.sh <name> <primary-path> [mode] [workstream_mode]`, adding
