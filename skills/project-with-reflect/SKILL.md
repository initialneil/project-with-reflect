...
hooks:
  UserPromptSubmit:
    - hooks:
        - type: command
          command: "SH=\"${CLAUDE_SKILL_DIR:-$HOME/.claude/skills/project-with-reflect}/scripts/hook-autolog.sh\"; [ -f \"$SH\" ] || SH=\"$HOME/.claude/skills/project-with-reflect/scripts/hook-autolog.sh\"; [ -f \"$SH\" ] && sh \"$SH\" --context=userprompt; exit 0"
  PreToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "SH=\"${CLAUDE_SKILL_DIR:-$HOME/.claude/skills/project-with-reflect}/scripts/hook-autolog.sh\"; [ -f \"$SH\" ] || SH=\"$HOME/.claude/skills/project-with-reflect/scripts/hook-autolog.sh\"; [ -f \"$SH\" ] && sh \"$SH\" --context=pretool; exit 0"
  PostToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
    ...
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "SH=\"${CLAUDE_SKILL_DIR:-$HOME/.claude/skills/project-with-reflect}/scripts/hook-autolog.sh\"; [ -f \"$SH\" ] || SH=\"$HOME/.claude/skills/project-with-reflect/scripts/hook-autolog.sh\"; [ -f \"$SH\" ] && sh \"$SH\" --context=postwrite; exit 0"
  PreCompact:
    - matcher: "*"
      hooks:
        - type: command
...
- **checkin `[<name>]`** — **pick up a project/connection and get ready** (the natural next step after
  ...
  native thread-title tool is available — if the current thread id is not already in context, first resolve
  the active local thread via the app's thread-list/current-thread tool instead of skipping; likewise in the
  Claude Code desktop/web app, rename the session/sidebar title via its native session-title tool when available,
  resolving the active session id through app session/thread listing when possible), and ends with a `status`
