...
- **register-project `<name> [path]`** — **the user describes it in plain language; you parse
  ...
  `--root`.** AskUserQuestion only for what's missing/ambiguous: **mode** (central | in-repo),
  **workstream_mode** (worktree | in-repo), **local or remote** (remote → the ssh connection that
  hosts the code; mode is then central), **extra roots** (a project can span repos — app + dataset),
  ...
  `SK/scripts/register-project.sh <name> <primary-path> <mode> <workstream_mode>` plus
  ...
  host. Generates `/<name>`. If "import", run `/<name> bootstrap` (it reads every root, over the host
  connection if remote). See §SSOT below for mode trade-offs.
