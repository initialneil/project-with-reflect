...
TOOLCHAIN="${4:-arduino-cli}"; BAUD="${5:-115200}"; FLASH_CMD="${6:-}"; MONITOR_CMD="${7:-}"
CDIR="$PWR_ROOT/connections/$NAME"; mkdir -p "$CDIR"

python3 - "$CDIR/connection.json" "$NAME" "$BOARD" "$PORT" "$TOOLCHAIN" "$BAUD" "$FLASH_CMD" "$MONITOR_CMD" <<'PY'
import json, sys
path, name, board, port, tc, baud, flash, monitor = sys.argv[1:9]
json.dump({"name": name, "transport": "serial", "board": board, "port": port,
...
python3 "$HERE/_note.py" "$CDIR/$NAME.md" "$NAME" connection \
  ...
  flash="$FLASH_CMD" monitor="$MONITOR_CMD"
...
echo "Registered serial connection '$NAME' ($BOARD on $PORT, $TOOLCHAIN) — skill at ~/.claude/skills/$NAME."
...
echo "Use /$NAME flash | monitor | reconnect wifi | repl. Run /reload-plugins to load it now."
